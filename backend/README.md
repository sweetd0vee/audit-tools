...
`аудиторское мнение` — черновик раздела I заключения (2–4 стр., без таблиц) из **подтверждённых** гипотез. Сначала `утверждаю гипотезы 1, 3, 5`. Свои гипотезы — `POST …/hypotheses/select` с полем `extra` (.xlsx) или `extra_hypotheses` в JSON. Шрифт: `-c` / `font=c` Calibri, `-t` / `font=t` Times New Roman.
