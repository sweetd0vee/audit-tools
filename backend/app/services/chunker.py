...
_HEADING_LINE_RE = re.compile(
    r"(?im)^[ \t]*(?:Статья|Ст\.|Глава|ГЛАВА|СТАТЬЯ|Раздел|РАЗДЕЛ)\s+\d+(?:\.\d+)?\b.*$"
)


def normalize_npa_text(text: str) -> str:
    """Prefix article/chapter lines with markdown headings for chunkers and BM25."""
    raw = (text or "").replace("\r\n", "\n")
    ...
    return _HEADING_LINE_RE.sub(_heading, raw)
